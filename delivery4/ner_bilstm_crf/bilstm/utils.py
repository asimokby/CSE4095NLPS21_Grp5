...
def load_tr_vectors(filename):
    ...
    for line in open(filename, 'r'):
        ...
        word = line[0]
        vectors = np.array([float(val) for val in line[1:]])
...
def save_params(filename, params):
    with open(filename, 'w') as params_file:
        ...
        params_file.write(saving_time + "\n\n")

        for param in params:
            params_file.write(param + ": " + str(params[param]) + "\n")
