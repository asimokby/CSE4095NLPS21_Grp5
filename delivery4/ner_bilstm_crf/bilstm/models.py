...
from keras.layers import Dense, LSTM, Bidirectional, Embedding, Input, Dropout, Lambda, \
    Activation, Reshape, TimeDistributed, Conv1D, GlobalMaxPooling1D, MaxPooling1D
from keras.layers.merge import Concatenate
from keras.models import Model

from bilstm.layers import CRF
...
class BiLSTMCRF(BaseModel):
    ...
    def __init__(self,
        ...
        self._use_extra_layer = use_extra_layer
    ...
    def build(self):
        ...
        if self._use_char:
            ...
            char_embeddings = Lambda(
                lambda x: K.reshape(x, shape=(
                    -1, s[-2], self._char_embedding_dim)))(char_embeddings)

            if not self._use_cnn:
                fwd_state = LSTM(self._char_lstm_size,
                                 return_state=True)(char_embeddings)[-2]
                bwd_state = LSTM(self._char_lstm_size, return_state=True,
                                 go_backwards=True)(char_embeddings)[-2]
                char_embeddings = Concatenate(axis=-1)([fwd_state, bwd_state])
                char_embeddings = Lambda(
                    lambda x: K.reshape(x, shape=[
                        -1, s[1], 2 * self._char_lstm_size]))(char_embeddings)
            else:
                char_embeddings = Dropout(self._dropout)(char_embeddings)
                char_embeddings = Conv1D(self._filter_size,
                                         self._filter_length,
                                         padding='same')(char_embeddings)
                char_embeddings = GlobalMaxPooling1D()(char_embeddings)
                char_embeddings = Lambda(
                    lambda x: K.reshape(x, shape=[
                        -1, s[1], self._filter_size]))(char_embeddings)
        ...
        if self._use_crf:
            crf = CRF(self._num_labels, learn_mode=self._learn_mode,
                      sparse_target=False)
