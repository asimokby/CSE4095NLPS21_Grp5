...
"""
from bilstm.utils import batch_iter
from bilstm.callbacks import F1score, TestCallback
...
class Trainer(object):
    ...
    def train(self, x_train, y_train, x_valid=None, y_valid=None,
        ...
        if x_test and y_test:
            test_steps, test_generator = batch_iter(x_test, y_test,
                                                    batch_size,
                                                    shuffle=False,
                                                    preprocessor=self._preprocessor)
            f1 = TestCallback(test_steps, test_generator,
                              preprocessor=self._preprocessor)
            callbacks = [f1] + callbacks if callbacks else [f1]
